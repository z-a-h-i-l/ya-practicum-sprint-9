...
- **Реализовано кэширование отчётов в S3** — при первом запросе отчёт генерируется из ClickHouse, сохраняется в Minio, а при повторных — сервис сразу возвращает CDN-ссылку без обращения к БД.

- **Добавлена CDN на базе Nginx** — поднят reverse proxy с кэшированием, который раздаёт отчёты из Minio и снижает нагрузку на хранилище.

- **Продумана инвалидация кэша** — при `force_refresh` сервис отправляет PURGE-запрос в Nginx и добавляет в URL версионный параметр `?v=timestamp`, гарантируя обновление.

- **Оптимизирована структура хранения** — ключи в S3 имеют вид `{customer_id}/{period_start}_{period_end}.json`, что позволяет быстро искать отчёты по клиенту.

- **Расширена конфигурация Docker Compose** — добавлены сервисы `minio`, `minio-init`, `cdn`, а в `reports-api` прописаны переменные окружения для подключения к S3 и CDN.

- **Добавлен эндпоинт для отладки** — `/reports/direct` возвращает отчёт напрямую из ClickHouse, минуя кэш, что полезно при диагностике.

# Задание 4

- **Внедрена CRM PostgreSQL с логической репликацией** — добавлен сервис `crm-db` с параметрами `wal_level=logical` и слотами репликации, что позволяет Debezium захватывать изменения без блокировок OLTP-запросов.

- **Развёрнуты Kafka и Zookeeper** — обеспечена надёжная доставка CDC-событий от Debezium в брокер сообщений для последующей асинхронной обработки.

- **Добавлен Kafka Connect с коннектором Debezium** — сервис `kafka-connect` настроен на чтение изменений из CRM PostgreSQL и публикацию их в топики Kafka, разгружая транзакционную БД от тяжёлых выгрузок.

- **ClickHouse переключён на потребление из Kafka** — добавлены `depends_on: kafka` и скрипты инициализации (`clickhouse/init`); данные из топиков забираются через движок KafkaEngine, а витрина `report_mart` обновляется инкрементально с помощью MaterializedView.